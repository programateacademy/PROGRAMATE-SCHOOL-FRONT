...
const Input = ({ state, changeStatus, type, label, placeholder, id, error, question, funcion  }) => {
    ...
    return (
        <div className='w-80 mx-7 pb-6'>
            <label className='pb-1.5 text-dark text-sm font-Nunito font-black' htmlFor={id}
            validated={state.validated}
            >{label}</label>
            <div>
                <input className='w-full px-1 py-1 rounded border-2 border-yellow text-dark/50 text-xs font-Poppins outline-none'
                    type={type}
